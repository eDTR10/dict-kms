...
const ABOUT_LINKS = [
  { label: 'Mandate, Powers and Functions', tab: 'mandate' },
  { label: 'Mission and Vision', tab: 'mission' },
  { label: 'DICT Quality Policy', tab: 'quality' },
  { label: "Citizen's Charter 2025", tab: 'charter' },
];

const PROJECT_LINKS = [
  { label: 'Free Wi-Fi', path: '/projects/free-wifi' },
  { label: 'ILCDB', path: '/projects/ilcdb' },
  { label: 'eGov (NGP)', path: '/projects/egov' },
  { label: 'Cybersecurity / PNPKI', path: '/projects/cybersecurity' },
  { label: 'NBP / CDO GovNet', path: '/projects/govnet' },
  { label: 'eLGU', path: '/projects/elgu' },
  { label: 'IIDB', path: '/projects/iidb' },
  { label: 'NIPPSB', path: '/projects/nippsb' },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <Link
              to="/"
              className={`px-3 py-2 rounded-md text-sm font-medium transition-colors hover:bg-white/10 ${isActive('/') ? 'bg-white/20' : ''}`}
            >
              Home
            </Link>
            ...
            <div className="relative" ref={aboutRef}>
              ...
              {aboutOpen && (
                <div className="absolute top-full left-0 mt-1 w-56 bg-white dark:bg-gray-800 rounded-lg shadow-xl py-1 z-50">
                  {ABOUT_LINKS.map(({ label, tab }) => (
                    <Link
                      key={tab}
                      to={`/about?tab=${tab}`}
                      onClick={() => setAboutOpen(false)}
                      className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-[#e8edf7] dark:hover:bg-gray-700 transition-colors"
                    >
                      {label}
                    </Link>
                  ))}
            ...
            <div className="relative" ref={projectsRef}>
              <button
                ...
                className={`flex items-center gap-1 px-3 py-2 rounded-md text-sm font-medium transition-colors hover:bg-white/10 ${
                  location.pathname.startsWith('/projects') ? 'bg-white/20' : ''
                }`}
              >
                Projects
              ...
              {projectsOpen && (
                <div className="absolute top-full left-0 mt-1 w-52 bg-white dark:bg-gray-800 rounded-lg shadow-xl py-1 z-50">
                  {PROJECT_LINKS.map((child) => (
                    <Link
                      ...
                      className={`block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-[#e8edf7] dark:hover:bg-gray-700 transition-colors ${
                        isActive(child.path) ? 'text-[#0038A8] font-semibold bg-[#e8edf7] dark:bg-gray-700' : ''
                      }`}
                    >
                      {child.label}
                    </Link>
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <div className="relative" ref={userMenuRef}>
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl py-1 z-50">
        ...
        {mobileOpen && (
          <div className="md:hidden pb-4 border-t border-white/20 mt-2 pt-2 space-y-1">
            <Link to="/" onClick={() => setMobileOpen(false)}
              className={`block px-3 py-2 rounded-md text-sm hover:bg-white/10 ${isActive('/') ? 'bg-white/20' : ''}`}>
              Home
            </Link>
            ...
            {ABOUT_LINKS.map(({ label, tab }) => (
              <Link key={tab} to={`/about?tab=${tab}`} onClick={() => setMobileOpen(false)}
